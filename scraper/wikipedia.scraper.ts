...
export class Wikipedia {
  ...
  static async scrapePage(browser: Browser, page: Page) {
    ...
    // }
    if (h1.includes("Search") && !h1.includes("disambiguation")) {
    ...
    if (h1.includes("disambiguation")) {
      ...
      const show = res.find(
        (r) =>
          r.toLowerCase().includes("tv") && r.toLowerCase().includes("series")
      )
      if (show) {
  ...
  static async searchQuery(browser: Browser, query: string) {
    ...
    await delay(500)
    const opts = await page.$$("a.cdx-menu-item__content")
    for (const opt of opts) {
      const href = await opt.evaluate((a) => a.href)
      if (href.toLowerCase().includes("tv")) {
        await page.close()
        const newPage = await browser.newPage()
